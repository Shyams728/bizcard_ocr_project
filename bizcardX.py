...
# Initialize the session state with the current index
...
def delete_current_card(sqlite_conn, current_row):
    ...
    if st.session_state['current_index'] >= len(data):
        st.session_state['current_index'] = max(0, len(data) - 1)
...
def data_card():
    st.title('Business Card Information Viewer')
    ...
    if not data.empty:
        ...
        if col1.button('< Previous') and st.session_state['current_index'] > 0:
            st.session_state['current_index'] -= 1
        if col4.button('Next >') and st.session_state['current_index'] < len(data) - 1:
            st.session_state['current_index'] += 1

        # Display the button to show the business card image
        if col2.button("Show Business Card Image"):
            display_bizcard_image(current_row)

        # Delete button
        if col3.button('Delete Card'):
...
def main():
    ...
    if selcted_option == "***View & Modify The Data***":
        ...
        # display_main.dataframe(bizcard_data)
        with display_main:
